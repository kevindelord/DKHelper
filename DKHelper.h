...
#define OBJECT(dictionary, key)                     [dictionary objectForKey:key]
#define VALID(dict, key)                            (dict && OBJECT(dict, key) && ![OBJECT(dict, key) isEqual:[NSNull null]])
#define GET_NUMBER(dictionary, key)                 ( VALID(dictionary, key) ? [NSNumber numberWithInteger:[OBJECT(dictionary, key) integerValue]] : nil )
#define GET_DATE(dictionary, key)                   ( VALID(dictionary, key) ? [NSDate dateFromString:OBJECT(dictionary, key)] : nil )
#define GET_STRING(dictionary, key)                 ( VALID(dictionary, key) ? ( ![OBJECT(dictionary, key) isEqualToString:@""] ? OBJECT(dictionary, key) : nil ) : nil )
#define GET_ASSET_URL(dictionary, key1, key2)       ( VALID(dictionary, key1) ? ( VALID(OBJECT(dictionary, key1), key2) ? OBJECT(OBJECT(dictionary, key1), key2) : nil ) : nil )
