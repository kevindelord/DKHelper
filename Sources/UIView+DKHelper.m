...
+ (instancetype _Nullable)loadFromNib:(NSString * _Nonnull)name {
	return [self loadFromNib:name fromBundle:[NSBundle mainBundle]];
}

+ (instancetype _Nullable)loadFromNib:(NSString * _Nonnull)name fromBundle:(NSBundle * _Nonnull)bundle {
	if ([bundle URLForResource:name withExtension:@".xib"] != nil ||
		[bundle URLForResource:name withExtension:@".nib"] != nil) {
		NSArray *nibs = [bundle loadNibNamed:name owner:nil options:nil];
